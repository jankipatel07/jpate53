 ...
 * DESCRIPTION: Tetro_J.java : Class which will implement TetroShape ---> for 'J' 
 ********************************************************************************/
public class Tetro_J extends Tetromino implements TetroShape {
	 ...
	 public boolean isGridFull(block grid[][]) {
	    ...
	    public boolean canMove(block grid[][],int r,int c) {
			for(int i=0; i<4; i++) {
				for(int j=0; j<4; j++) {
					...
					if(shapeArea[i][j] == 'j') { // piece contains this square?
		...
		public void cut(block grid[][]) {
			...
			for(int i=0; i<4; i++)
				for(int j=0; j<4; j++)
				{	
					if(shapeArea[i][j] == 'j' && position.y+i>=0)
					{
		 ...
		 */
		ImageIcon imageIcon = new ImageIcon("resources/brown.jpg");
			...
			for(int i=0; i<4; i++)
				for(int j=0; j<4; j++)
					if(shapeArea[i][j] == 'j' && position.y+i>=0){
						...
						into[position.y + i][position.x + j].changeletter("j");
