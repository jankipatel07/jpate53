 ...
 * DESCRIPTION: Tetro_O.java : Class which will implement TetroShape ---> for 'O' 
 ********************************************************************************/
public class Tetro_0 extends Tetromino implements TetroShape {
	
	 //private Point position = new Point(3, -4); // -4 to start above top row
	 private char shapeArea[][] = {{'x', 'x', 'x', 'x'},
            					   {'x', 'o', 'o', 'x'},
                                   {'x', 'o', 'o', 'x'},
                                   {'x', 'x', 'x', 'x'}};
    ...
    public boolean canMove(block grid[][],int r,int c) {
		for(int i=0; i<4; i++) {
			for(int j=0; j<4; j++) {
				...
				if(shapeArea[i][j] == 'o') { // piece contains this square?
	...
	public void cut(block grid[][]) {
		...
		for(int i=0; i<4; i++)
			for(int j=0; j<4; j++)
			{	
				if(shapeArea[i][j] == 'o' && position.y+i>=0)
				{
	 ...
	 */
	ImageIcon imageIcon = new ImageIcon("resources/magenta.jpg");
		...
		for(int i=0; i<4; i++)
			for(int j=0; j<4; j++)
				if(shapeArea[i][j] == 'o' && position.y+i>0){
					...
					into[position.y + i][position.x + j].changeletter("i");
